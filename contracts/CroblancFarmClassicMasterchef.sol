...
import "./CroblancTreasury.sol";

//----------------------------------------------------------------------------------------------------------------------
//        _
//       / \      _-'
//     _/|  \-''- _ /
//__-' { |          \        *** CroblancFarmClassicMasterchef ***
//    /             \
//    /       "O.  |O }      Generic farm template for third-parties based on Sushiswap, such as CronaSwap, VVSFinance
//    |            \ ;       Crodex. Yes, you see how diversified this ecosystem is... 🤠
//                  ',
//       \_         __\      This is a Classic farm: farm CRONA, distribute CRONA / farm VVS, distribute VVS, etc.
//         ''-_    \.//      However, some functions are renamed and depends of the project.
//           / '-____'       pendingSushi() becomes pendingCrona() or pendingVVS(), thus this contract is made abstract
//          /                and must be extended and override our pendingRewardsInMasterChef() with the proper name.
//        _'
//      _-'                  /!\ NO SUPPORT OF DEFLATIONARY TOKENS IN THIS VERSION /!\
//----------------------------------------------------------------------------------------------------------------------
abstract contract CroblancFarmClassicMasterchef is ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public minimumWheatToSwap;  // Minimum amount of wheat that must be pending in order to swap the performance fee into WCRO
    ...
    CroblancDividends public dividends;   // xCROBLANC
    CroblancTreasury public treasury;     // Treasury
    ...
    address[] public wheatToWcro;
    uint256 public performanceFeePendingSwap;
    ...
    constructor (
        ...
        treasury = CroblancTreasury(_treasury);
        want = IERC20(_want);
        wheat = IERC20(_wheat);
        ...
        // https://github.com/cronaswap/cronaswap-contracts
        // https://docs.vvs.finance/fundamentals/smart-contracts-and-security
        router = IUniswapV2Router02(_router);

        // There is always a direct pool between CRO <-> CRONA|VVS|CROW
        wheatToWcro = [_wheat, wcroAddress];
    ...
    // View function to see pending CRONAs on frontend, net of the performance fee.
    function pendingWheat(address _user) public view returns (uint256) {
        ...
        if (masterChefEnabled) {
            wheatHarvestable = pendingRewardInMasterChef().mul(userBalance).div(wantBalance);
        }

        return wheatAlreadyHarvested.add(wheatHarvestable).mul(alpha.performanceFeePerThousand()).div(1000);
    }
    ...
    function updatePool(bool _enableDividendsAutoswap) public whenNotPaused {
        ...
        if (_enableDividendsAutoswap && masterChefEnabled && pendingRewardInMasterChef() > 0) {
            ...
            performanceFeePendingSwap = performanceFeePendingSwap.add(performanceFee);

            // Autoswap to dividends?
            if (performanceFeePendingSwap >= minimumWheatToSwap) {
                // Swap performance fees!
                // Convert all performance fee in WCRO
                router.swapExactTokensForTokens(performanceFeePendingSwap, 0, wheatToWcro, address(this), block.timestamp);

                // Make dividend contract pull a 2/3 of the WCROs
                dividends.pullDividends(address(this), wcro.balanceOf(address(this)).mul(6666).div(10000));

                // Send the remaining to treasury
                wcro.transfer(address(treasury), wcro.balanceOf(address(this)));

                performanceFeePendingSwap = 0;
            }

            // Count rewards for wheat distribution
            uint256 wheatRedistributed = wheatHarvested.sub(performanceFee);
            if (wheatRedistributed > 0) {
                accWheatPerShare = accWheatPerShare.add(wheatHarvested.mul(1e18).div(wantBalance));
            }
        }
    ...
    function setMinimumWheatToSwap(uint256 _minimumWheatToSwap) external onlyOwner {
        minimumWheatToSwap = _minimumWheatToSwap;
    }
